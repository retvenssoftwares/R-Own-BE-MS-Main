...
//     const searchQuery = {
//       user_id: User_id,
//       post_type: { $in: ['click and share', 'share some media'] }
//     };
...
//     const likedPosts = await Like.find({ post_id: { $in: postIds }, "likes.user_id": user_id });
//     //const commentPosts = await comment.find({ post_id: { $in: postIds }, "comments.user_id": user_id });
//     //const likedPosts = await Like.find({ post_id: { $in: postIds } });
//     const commentPosts = await comment.find({ post_id: { $in: postIds } });
//     const likedPostIds = likedPosts.map((likedPost) => likedPost.post_id);
//     //const commentPostIds = commentPosts.map((commentPost) => commentPost.post_id);
...
//       return { ...post.toObject(), saved: isSaved ? 'saved' : 'not saved',liked: isLiked ? 'liked' : 'not liked', likeCount: likeCount,commentCount:commentCount };
//     });
